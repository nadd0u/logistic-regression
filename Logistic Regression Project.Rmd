...
```{r}
evaluation <- read.csv(file="file:///Users/nadianoui-mehidi/Desktop/logit_insurance_test.csv",head=TRUE,sep=",")
training <- read.csv(file="file:///Users/nadianoui-mehidi/Desktop/logit_insurance.csv",head=TRUE,sep=",")
...
train <- read.csv(file="file:///Users/nadianoui-mehidi/Desktop/logit_insurance.csv",head=TRUE,sep=",")%>%
...
# Introduction
In this project we are predicting whether or not an insurance policy customer is likely to be involved in a car accident. Then, if it is determined that the customer is likely to be involved in a car accident, we provide a predictive estimate of the likely cost of the subsequent auto insurance claim.
...
mytable <- table(train$target_flagg, train$job, dnn = "Observations by job")

prop.table(mytable)
mytable2 <- table(train$target_flagg, train$education, dnn = "Observations by education")

prop.table(mytable2)
mytable3 <- table(train$target_flagg, train$car_type,dnn = "Observations by car type")
...
singleparent<-table(data0$target_flagg,data0$parent1)
...
sex<-table(data0$target_flagg,data0$sex)
...
drivingchildren<-table(data0$target_flagg,data0$kidsdriv)
...
model = subset(complete, select = -c(travtime, kidsdriv,job, index, age, car_age, income, yoj, oldclaim, homekids, education,car_type,target_flagg) )
...
a <- glm(target_flag ~parent1+home_val+mstatus+sex+car_use+ bluebook+ tif+red_car+ clm_freq+ revoked+ mvr_pts+ urbanicity+ highincome+ young+ college+ bluecollar+ student+ minivan+ newjob+ pastclaim+ kids+ kidsdrive+ newcar+longcommute, model,family=binomial("logit"))
summary(a)
...
c <- step(glm.null, direction = "forward", trace = 1, scope = ~ parent1+home_val+mstatus+sex+car_use+ bluebook+ tif+red_car+ clm_freq+ revoked+ mvr_pts+ urbanicity+ highincome+ young+ college+ bluecollar+ student+ minivan+ newjob+ pastclaim+ kids+ kidsdrive+ newcar+longcommute)
```
